...
import { PACKAGES, STYLE_GOALS, QUESTION_CHIPS } from "../lib/content";
...
const TOTAL = STEP_TITLES.length; // 6 content steps

/* ── Helpers ───────────────────────────────────────────────────── */
const emailValid = (e: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e);

/** sample average luminance of an image to offer a gentle lighting hint */
...
function StepShell({
  ...
  return (
    <div className="step-anim">
      ...
      {intro && <p className="mt-3 max-w-xl text-ink-soft">{intro}</p>}
...
export default function IntakeStudio() {
  ...
  /* hydrate text fields from localStorage (photos are never persisted) */
  useEffect(() => {
    try {
  ...
  /* autosave (debounced-ish via effect) + a gentle toast */
  useEffect(() => {
    if (!started) return;
    try {
  ...
  const submit = () => {
    ...
    // Front-facing demo: nothing is actually sent. Clear the saved draft.
    try {
  ...
  /* ───────────────── Completion ───────────────── */
  if (done) {
    return <Completion firstName={data.firstName} />;
  }
    ...
    return (
      <div className="step-anim mx-auto max-w-2xl text-center">
        <div className="mx-auto mb-8 flex justify-center gap-2">
          {["#f2a98c", "#cf97a8", "#b9763f", "#5b3a59", "#c9a86a"].map((c) => (
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm text-ink-soft">
    ...
    <div ref={topRef} className="mx-auto max-w-2xl scroll-mt-28">
      ...
      {step === 1 && (
        <StepShell
          ...
          title={<>First, who am I meeting?</>}
          intro="Just enough to reach you and to know which experience you're curious about."
        >
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              ...
              {showErrors && !data.firstName.trim() && (
                <p className="mt-2 text-sm text-rose-deep">A first name helps me greet you properly.</p>
              )}
            </div>
            <div>
              ...
              />
              {showErrors && !emailValid(data.email) && (
                <p className="mt-2 text-sm text-rose-deep">A valid email so I can send your next steps.</p>
              )}
            </div>
          </div>
          <div>
          ...
          <div>
            <span className="field-label">Which experience are you curious about?</span>
            <div className="flex flex-wrap gap-2.5">
              {[...PACKAGES.map((p) => p.name.replace("The ", "")), "Just exploring"].map((p) => (
                <button
                  key={p}
                  type="button"
                  className="chip"
                  aria-pressed={data.pkg === p}
                  onClick={() => set("pkg", data.pkg === p ? "" : p)}
                >
                  {p}
                </button>
              ))}
      ...
      {step === 2 && (
        ...
        >
          <div
            ...
            style={{ background: "var(--color-blush)", color: "var(--color-ink)" }}
          >
            <strong className="font-semibold">A quick word on light.</strong>{" "}
            Natural daylight tells the truth; lamps and filters fib. No need to be perfect —
            I&rsquo;ll gently flag anything too dark or backlit.
          </div>
              ...
              return (
                <div key={slot.key} className="flex flex-col">
                  <label
                    ...
                    style={{
                      borderColor: "color-mix(in srgb, var(--color-gild) 45%, transparent)",
                      background: photo ? "transparent" : "#fff",
                    }}
                  >
                    {photo ? (
                      ...
                      <span className="px-3">
                        <span className="block text-2xl text-rose-deep">＋</span>
                        <span className="mt-2 block text-sm font-medium">{slot.title}</span>
                        <span className="mt-1 block text-xs text-ink-soft">{slot.help}</span>
                      </span>
                    )}
                    <input
                      type="file"
                      ...
                      capture="user"
                      className="sr-only"
                      onChange={(e) => onPhoto(slot.key, e.target.files?.[0])}
                    />
                  </label>
                  {photo && (
                  ...
                  {photo?.hint && (
                    <p className="mt-1 text-xs" style={{ color: "var(--color-laurel)" }}>
                      {photo.hint}
                    </p>
                  )}
                </div>
      ...
      {step === 3 && (
        <StepShell
          eyebrow="Your style goals"
          title={<>What would feel like a win?</>}
          intro="Pick anything that resonates — there are no wrong answers."
        >
          <Chips
            options={STYLE_GOALS}
            selected={data.styleGoals}
            onToggle={(v) => toggleIn("styleGoals", v)}
          />
          <div>
            <label className="field-label" htmlFor="goalsText">
              Anything else you&rsquo;re hoping for? <span className="font-normal text-ink-soft">(optional)</span>
            </label>
            <textarea
              id="goalsText"
              className="field min-h-28 resize-y"
              placeholder="In your own words…"
              value={data.styleGoalsText}
              onChange={(e) => set("styleGoalsText", e.target.value)}
            />
          </div>
        </StepShell>
      )}
      ...
      {step === 4 && (
        <StepShell
          eyebrow="Your colors today"
          title={<>How do you dress right now?</>}
          intro="A little context primes our session — none of this is binding."
        >
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              ...
              <input
                id="love"
                className="field"
                placeholder="e.g. olive, rust, cream"
                value={data.colorsLove}
                onChange={(e) => set("colorsLove", e.target.value)}
              />
            </div>
            <div>
              ...
              <input
                id="avoid"
                className="field"
                placeholder="e.g. neon, icy pastels"
                value={data.colorsAvoid}
                onChange={(e) => set("colorsAvoid", e.target.value)}
              />
            </div>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            <div>
              ...
              <div className="flex flex-wrap gap-2.5">
                {["Natural", "Colored", "A mix"].map((h) => (
                  <button
                    ...
                    type="button"
                    className="chip"
                    aria-pressed={data.hair === h}
                    onClick={() => set("hair", data.hair === h ? "" : h)}
                  >
                    {h}
                  </button>
                ))}
              ...
              <input
                ...
                placeholder="Current shade (optional)"
                value={data.hairShade}
                onChange={(e) => set("hairShade", e.target.value)}
              />
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2.5">
                {["No, first time", "Yes"].map((p) => (
                  <button
                    ...
                    type="button"
                    className="chip"
                    aria-pressed={data.prior === p}
                    onClick={() => set("prior", data.prior === p ? "" : p)}
                  >
                    {p}
                  </button>
                ))}
              ...
              {data.prior === "Yes" && (
                <input
                  className="field mt-3 step-anim"
                  placeholder="What season were you told?"
                  value={data.priorSeason}
                  onChange={(e) => set("priorSeason", e.target.value)}
                />
              )}
            </div>
          </div>
      ...
      {step === 5 && (
        <StepShell
          ...
          title={<>What do you most want to ask?</>}
          intro="The session is yours. Tell me what's on your mind so we spend our time on what matters to you."
        >
          <div>
          ...
          <div>
            ...
            <Chips
              options={QUESTION_CHIPS}
              selected={data.questionChips}
              onToggle={(v) => toggleIn("questionChips", v)}
            />
          </div>
          <div>
            <label className="field-label" htmlFor="heard">
              How did you hear about the studio? <span className="font-normal text-ink-soft">(optional)</span>
            </label>
            <input
              id="heard"
              className="field"
      ...
      {step === 6 && (
        <StepShell
          eyebrow="Review & send"
          title={<>Here&rsquo;s what I&rsquo;ll see.</>}
          intro="Take a look, edit anything, and send it to the studio when it feels right."
        >
          <ReviewRow label="About you" onEdit={() => jumpTo(1)}>
            {data.firstName || "—"} · {data.email || "—"}
            {data.phone ? ` · ${data.phone}` : ""}
            {data.pkg ? ` · Curious about: ${data.pkg}` : ""}
          </ReviewRow>
          <ReviewRow label="Photos" onEdit={() => jumpTo(2)}>
            {photoCount > 0 ? (
              <span className="flex items-center gap-2">
                ...
                <span className="text-ink-soft">{photoCount} of 3 added</span>
              </span>
            ) : (
          ...
          <ReviewRow label="Colors today" onEdit={() => jumpTo(4)}>
            ...
            {data.prior ? `Prior analysis: ${data.prior}${data.priorSeason ? ` — ${data.priorSeason}` : ""}.` : ""}
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-2xl p-4" style={{ background: "var(--color-seaglass)" }}>
            <input
              type="checkbox"
              className="mt-1 h-5 w-5 accent-[var(--color-rose)]"
              checked={data.consent}
              onChange={(e) => set("consent", e.target.checked)}
            />
            <span className="text-sm text-ink-soft">
              I understand my photos and answers are used only to prepare my color analysis — never
              sold, never used for ads, never fed to a tracker.
            </span>
          </label>
          {showErrors && !data.consent && (
            <p className="text-sm text-rose-deep">Please confirm so I can prepare your analysis with care.</p>
          )}
        </StepShell>
      )}
...
function ReviewRow({
  ...
  return (
    <div
      ...
      style={{ borderColor: "color-mix(in srgb, var(--color-gild) 35%, transparent)" }}
    >
      <div className="flex items-center justify-between">
        <span className="text-[0.66rem] font-semibold uppercase tracking-[0.18em] text-ink-soft">
          {label}
        </span>
...
function Completion({ firstName }: { firstName: string }) {
  const gems = ["#f2a98c", "#cf97a8", "#c98b8b", "#b9763f", "#7c7a52", "#5b3a59", "#c9a86a"];
  return (
    <div className="step-anim relative mx-auto max-w-2xl overflow-hidden text-center">
      {/* a single tasteful flourish */}
      ...
      <div className="relative">
        ...
        <p className="mt-6 text-xs text-ink-soft">
          This is a demonstration intake — your details weren&rsquo;t actually sent anywhere.
        </p>
      </div>
    </div>
  );
